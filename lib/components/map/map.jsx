...
class LeafletMap extends React.Component {
  ...
  getWeights(){
    const ranks = ["crimes","transit", "restaurants"];
    ...
    ranks.forEach((filter, idx) => {
      if (ranks.length === 3) {
        const weights = [.5, .3, .2]
        eval("intensity" + filter + `=${3 * weights[idx]}`);
      }
  ...
  getIntensity(crimeIntensity, transitIntensity, restaurantIntensity){
    ...
    this.state.crimeFavorabilityScore = (1/this.props.allCrimes.length) * minimum * crimeIntensity
    this.state.transitFavorabilityScore = (1/this.props.allTransit.length) * minimum * transitIntensity
    this.state.restaurantFavorabilityScore = (1/this.props.allRestaurants.length) * minimum * restaurantIntensity
  }
